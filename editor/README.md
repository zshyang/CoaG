...
2. In the editor: *import camera fit* (sets focal and pitch, height untouched, and stores the fit in `scene.camera.fit`), tick *anchor first frame to the photo* so frame 0 keeps the fitted camera exactly and orbit / pan / crane become rigid moves that start there (0..+40 deg orbit, 0..+24 deg pan, crane rising with the orientation kept), then *background image* to draw the photo under the preview (object-fit contain, grid drawn semi-transparent on top). A yellow dashed line marks the photo horizon from the fit and a blue dotted line the grid's own vanishing line: they should coincide.
...
Roll is reported but ignored (the renderer has no roll; pick photos with a level horizon). Without the anchor box, orbit and crane re-aim the camera at the people, so their first frame does not match the photo; with it, every path starts at the fitted camera. Only the file name is exported (`camera.background_name`), not the image.
