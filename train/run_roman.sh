#!/bin/bash
# George's live test (2026-09-13): two Roman soldiers fighting in the Colosseum, hand-authored geometry, several camera paths.
...
sel=("$@"); [ $# -eq 0 ] && sel=(orbit crane dolly_in static)
for p in "${sel[@]}"; do
  name=roman_$p; if ls "$WORK/out/samples/$name/"*.mp4 >/dev/null 2>&1; then echo "skip $name"; continue; fi
  echo "=== $name $(date -u +%H:%M)"
  CID=c0599 CV=$U/authored/roman_duel_$p/control.mp4 REF=$U/ref/colosseum.png PROMPT="$SCENE $PEOPLE $ACTION ${CAM[$p]}" TAG=$name $R > "$WORK/out/roman_$name.log" 2>&1 || echo "FAILED $name"
done; echo ROMAN_DONE
