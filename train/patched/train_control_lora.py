    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            for example in examples:
                ...
                if args.train_mode != "control":
                    # --- wan_control_demo patch: external reference image (metadata key ref_file_path) replaces the
                    # frame-of-the-clip reference. clip_idx = -1 marks "not a frame": the first-frame conv-in channel is
                    # zeroed and the full-reference self-attention path carries the image, matching ref_image at inference.
                    ref_ext = example.get("ref_pixel_values_ext", None)
                    use_upstream_ref = ref_ext is None
                    if not use_upstream_ref:
                        ref_t = torch.from_numpy(ref_ext).permute(2, 0, 1).contiguous().unsqueeze(0) / 255.
                        ref_t = transform(ref_t)
                        clip_index = -1
                        new_examples["clip_idx"].append(clip_index)
                        new_examples["ref_pixel_values"].append(ref_t)
                        clip_pixel_values = ref_t[0].permute(1, 2, 0).contiguous()
