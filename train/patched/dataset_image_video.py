...
class ImageVideoControlDataset(Dataset):
    ...
    def get_batch(self, idx):
        ...
        if data_info.get('type', 'image')=='video':
            ...
            # --- wan_control_demo patch: optional external reference image (a person-free background PNG).
            # Returned at the raw frame size so collate_fn can apply the same resize/crop as the frames.
            ref_ext = None
            ref_path = data_info.get('ref_file_path')
            if ref_path is not None:
                ref_path = ref_path if self.data_root is None else os.path.join(self.data_root, ref_path)
                ref_img = Image.open(ref_path).convert('RGB')
                if self.enable_bucket:
                    rh, rw = pixel_values.shape[1:3]
                    ref_ext = np.array(ref_img.resize((rw, rh), Image.BILINEAR))
                else:
                    rh, rw = pixel_values.shape[-2:]
                    ref_ext = torch.from_numpy(np.array(ref_img.resize((rw, rh), Image.BILINEAR))).permute(2, 0, 1).unsqueeze(0) / 255.
                    ref_ext = self.video_transforms(ref_ext)[0]

            return pixel_values, control_pixel_values, subject_image, control_camera_values, text, "video", ref_ext
        else:
            # Load and preprocess image
            image_path, text = data_info['file_path'], data_info['text']
            if self.data_root is not None:
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            try:
                ...
                pixel_values, control_pixel_values, subject_image, control_camera_values, name, data_type, ref_ext = self.get_batch(idx)
                sample["ref_pixel_values_ext"] = ref_ext
